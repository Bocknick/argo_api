async function get_wmos(goShip_only,max_dist){
  ...
  wmo_list = meta_data.map(row => row.WMO);
  go_ship_list = meta_data.map(row => row.GO_SHIP);
  if(goShip_only){
    wmo_list = wmo_list.filter((value,i)=>go_ship_list[i]===true)
  }
...
async function get_map_data(selected_param,max_dist){
  ...
  try {
    // fetch(node_route) returnsc a promise. WHAT IS A PROMISE? By placing
    ...
    max_dist = max_dist.toString();
    const response = await fetch(node_route, 
      {method: 'POST', 
       headers:  { 'Content-Type': 'application/json' },
       body: JSON.stringify({selected_param,max_dist})}) ;
...
function make_plot(plot_data,plot_type,selected_wmos,do_log,do_reg){
  ...
  let cruise_data = plot_data.selected_data.map(row => row.CRUISE);
